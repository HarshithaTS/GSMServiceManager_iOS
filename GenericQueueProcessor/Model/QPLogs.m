...
{
    NSString *logFolderPath = [self getSharedLogFolderComponent:@"QueueProcessorLogs"];
...
-(NSString *)getSharedLogFolderComponent :(NSString*)pathComponent
{
...
//    NSString *directoryToCreate = @"user_abc";
    ...
    NSString *dirPath = [sharedContainerPathLocation stringByAppendingPathComponent:pathComponent];
    
    NSString *logFilePath = [NSString stringWithFormat:@"%@/Log %@.txt",dirPath,currentDate];
    fileExists = [[NSFileManager defaultManager] fileExistsAtPath:logFilePath];
    ...
    if (![mgr fileExistsAtPath:dirPath isDirectory:&isdir] && isdir == NO) { //create a dir only that does not exists
    ...
    NSString *currentDateLogs = [dirPath stringByAppendingPathComponent:[NSString stringWithFormat:@"Log %@.txt",currentDate]];
