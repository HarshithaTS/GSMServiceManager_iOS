...
//    [self deleteItemsWithStatusDeleted];
    ...
    [self saveAllItemsInGSMKeyChainFromDb];
...
-(void)changePrioritytToLow{
   ...
   [[QPLogs sharedInstance]writeLogsToFile:[NSString stringWithFormat:@"\n %@ : Changing priority \n",[dateFormatter stringFromDate:[NSDate date]]]];
    ...
    NSMutableArray  *errorDictionary = [[NSMutableArray alloc]init];
    
    
    errorKeyChainArray = [GSPKeychainStoreManager getErrorItemsFromKeyChain];
    
    NSLog(@"Error items [changing priority] %@",errorKeyChainArray);
    ...
    for(id item in errorKeyChainArray){
        ...
        [errorDictionary addObject:dic];
    }
    
    NSLog(@"mutable dic [priority] %@",errorDictionary);
      
                    
            for(NSMutableDictionary *dict in errorDictionary){
                
                [dict setObject:@"3" forKey: @"periority"];
                        
                    }

    
    NSLog(@"Mutated Array %@",errorDictionary);
    [GSPKeychainStoreManager saveErrorItemsInKeychain:errorDictionary];
    errorKeyChainArray = [GSPKeychainStoreManager getErrorItemsFromKeyChain];
...
//         [[GQPQueueProcessor sharedInstance] saveDataToQueueDBFromKeyChain];
         [[GQPQueueProcessor sharedInstance]saveAllItemsInGSMKeyChainFromDb];
...
// Function added by Sahana on May 29th 2017 [alternative to saveAllItemsInKeyChainFromDb]
- (void)saveAllItemsInGSMKeyChainFromDb
{
    ...
    if (allItems.count > 0)
    {
        for (NSString *Field in allItems)
        {
            ...
            [allItemsLogDict setObject:[Field valueForKey:@"processCount"] forKey:@"processCount"];
              [allItemsLogDict setObject:[Field valueForKey:@"altID"] forKey:@"altID"];
              [allItemsLogDict setObject:[Field valueForKey:@"deviceID"] forKey:@"deviceID"];
              [allItemsLogDict setObject:[Field valueForKey:@"endTime"] forKey:@"endTime"];
              [allItemsLogDict setObject:[Field valueForKey:@"inputDataArrayString"] forKey:@"inputDataArrayString"];
            
            
            if([[Field valueForKey:@"packageName"]length]!=0)
                [allItemsLogDict setObject:[Field valueForKey:@"packageName"] forKey:@"packageName"];
            else
                [allItemsLogDict setObject:@""forKey:@"packageName"];

//              [allItemsLogDict setObject:[Field valueForKey:@"packageName"] forKey:@"packageName"];
              [allItemsLogDict setObject:[Field valueForKey:@"periority"] forKey:@"periority"];
              [allItemsLogDict setObject:[Field valueForKey:@"processStartTime"] forKey:@"processStartTime"];
            
            if([[Field valueForKey:@"queueDate"]length]!=0)
                [allItemsLogDict setObject:[Field valueForKey:@"queueDate"] forKey:@"queueDate"];
            else
                [allItemsLogDict setObject:@""forKey:@"queueDate"];
//              [allItemsLogDict setObject:[Field valueForKey:@"queueDate"] forKey:@"queueDate"];
              [allItemsLogDict setObject:[Field valueForKey:@"subApplication"] forKey:@"subApplication"];
              [allItemsLogDict setObject:[Field valueForKey:@"subApplicationVersion"] forKey:@"subApplicationVersion"];
